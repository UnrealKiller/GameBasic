 ...
 */
    ...
    */
    ...
    struct TMonoClass
    {
        ...
        {
            UScriptStruct* ScriptStruct = TScriptStructTraits<TRemoveReference<T>::Type>::Get();
            ...
            return FoundMonoClass;
        }

        static MonoObject* CreateObject(T* InSrcValue) {
            UScriptStruct* ScriptStruct = TScriptStructTraits<TRemoveReference<T>::Type>::Get();
            ...
            MonoObject* Object = FCSharpEnvironment::GetEnvironment().GetDomain()->Object_New(FoundMonoClass);
            FCSharpEnvironment::GetEnvironment().AddStructReference<false>(ScriptStruct, InSrcValue, Object);
    ...
    struct TMonoClass<T, true>
    {
        ...
        static MonoObject* CreateObject(T* InSrcValue) {
            return FCSharpEnvironment::GetEnvironment().Bind(InSrcValue);
        }
    };
    ...
    struct TPointerHelper<T, true>
    {
        static void* MakeArg(T* V)
        {
            return TMonoClass<TRemoveReference<T>::Type>::CreateObject(V);
        }
    };
    ...
    {
        static void* MakeArg(T&& V, bool bCopy)
        {
            if (bCopy)
            {
                return TMonoClass<TRemoveReference<T>::Type>::CreateObject(&V);
            }
            return TPointerHelper<typename TRemoveReference<T>::Type>::MakeArg(&V);
        }
    };
    ...
    template <typename T>
    void* MakeArg(T* V, bool bCopy = false) {
        return TPointerHelper<T>::MakeArg(V);
    }

    template <typename T>
    void* MakeArg(T&& V, bool bCopy) {
        return TGenericTypeHelper<T>::MakeArg(Forward<T>(V), bCopy);
    }

    template <bool bCopy> 
    void MakeArgs(TArray<void*>& OutArgs)
    {
    }

    template <bool bCopy, typename T1, typename... T2> 
    void MakeArgs(TArray<void*>& OutArgs, T1&& V1, T2&&... V2)
    {
        OutArgs.Emplace(MakeArg(Forward<T1>(V1), bCopy));
        MakeArgs<bCopy>(OutArgs, Forward<T2>(V2)...);
    }
}